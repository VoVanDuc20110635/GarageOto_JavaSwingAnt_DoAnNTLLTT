...
//            message.setContent(htmlMessage, "text/html");
    ...
    // gui phieu nhap hang
    public void sendEmailPhieuNhapHang(String to, String from, String host, String subject, PhieuNhapHang phieuNhapHang, List<ChiTietPhieuNhapHang> danhSachChiTietPhieuNhapHang ) {
        // Get system properties
        Properties properties = System.getProperties();
        properties.put("mail.smtp.host", "smtp.gmail.com");
        properties.put("mail.smtp.port", "587");
...
//            String htmlMessage = buildHtmlBill(customerName, items);
            String htmlMessage = buildHtmlPhieuNhapHang(phieuNhapHang, danhSachChiTietPhieuNhapHang);
//            message.setContent(htmlMessage, "text/html");
            message.setContent(htmlMessage, "text/html; charset=UTF-8");
            // Send message
            Transport.send(message);
    ...
    public String buildHtmlPhieuNhapHang(PhieuNhapHang phieuNhapHang, List<ChiTietPhieuNhapHang> danhSachChiTietPhieuNhapHang) {
    // Use StringBuilder to create the HTML content
        StringBuilder sb = new StringBuilder();
        sb.append("<html><head><meta charset='UTF-8'></head><body style='font-family: Arial, sans-serif; background-color: #f7f7f7; color: #333;'>");
        sb.append("<div style='max-width: 600px; margin: auto; background-color: #fff; padding: 20px; border: 1px solid #ddd;'>");
        ...
        sb.append("<p style='text-align: center;'>Thời gian in phiếu: 03/04/2024 22:20</p>");
        sb.append("<h1 style='background-color: #4A90E2; color: #fff; padding: 10px; text-align: center;'>PHIẾU TRẢ HÀNG</h1>");
        sb.append("<p>Mã phiếu:" + String.valueOf(phieuNhapHang.getPhieuNhapHang()) + "</p>");
        NhaCungCap nhaCungCap = new NhaCungCap();
        try {
            nhaCungCap = nhaCungCapService.hienThiNhaCungCapTheoMaNhaCungCap(phieuNhapHang.getMaNhaCungCap());
        } catch (SQLException ex) {
            Logger.getLogger(MailSender.class.getName()).log(Level.SEVERE, null, ex);
        }
        sb.append("<p>Nhà cung cấp: " + nhaCungCap.getTenNhaCungCap() + " - " + nhaCungCap.getDiaChi()+ "</p>"); 
        ...
        sb.append("<p>Người thực hiện: " + nhanVien.getTenNhanVien() + " - Mã nhân viên: " + nhanVien.getMaNhanVien() + "</p>");
        sb.append("<p>Thời gian nhập: " + util.localDateParseMethod(phieuNhapHang.getThoiGian()) + "</p>");
        sb.append("<p>Tổng thành tiền: <strong>" + String.valueOf(phieuNhapHang.getTong()) + "đ</strong></p>");
        sb.append("<br>");
        ...
        sb.append("<th style='padding: 8px; border: 1px solid #ddd;'>Mã hàng hóa</th>");
        sb.append("<th style='padding: 8px; border: 1px solid #ddd;'>Tên hàng hóa</th>");
        sb.append("<th style='padding: 8px; border: 1px solid #ddd;'>Giá giá nhập</th>");
        sb.append("<th style='padding: 8px; border: 1px solid #ddd;'>Số lượng</th>");
        sb.append("<th style='padding: 8px; border: 1px solid #ddd;'>Thành tiền</th>");
        sb.append("</tr>");
        
        for(int i =0; i< danhSachChiTietPhieuNhapHang.size(); i++){
            sb.append("<tr>");
            sb.append("<td style='padding: 8px; border: 1px solid #ddd;'>" + danhSachChiTietPhieuNhapHang.get(i).getMaHangHoa() + "</td>");
            sb.append("<td style='padding: 8px; border: 1px solid #ddd;'>" + danhSachChiTietPhieuNhapHang.get(i).getTenHangHoa()+ "</td>");
            sb.append("<td style='padding: 8px; border: 1px solid #ddd;'>" + String.valueOf(danhSachChiTietPhieuNhapHang.get(i).getGia_nhap()) + "đ</td>");
            sb.append("<td style='padding: 8px; border: 1px solid #ddd;'>" + String.valueOf(danhSachChiTietPhieuNhapHang.get(i).getSo_luong()) + "</td>");
            sb.append("<td style='padding: 8px; border: 1px solid #ddd;'>" + String.valueOf(danhSachChiTietPhieuNhapHang.get(i).getThanh_tien()) + "</td>");
            sb.append("</tr>");
        }
        sb.append("</table>");
        sb.append("<br>");
